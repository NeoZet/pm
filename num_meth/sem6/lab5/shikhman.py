...
def euler():
    ...
    while k < 2:
        ...
        t_k = t_k_next
        k += 1
    return y_k_prev_2, y_k_prev, y_k, y_k_next, tau_k_prev_2, tau_k_prev, tau_k
...
def shikhman():
    ...
    tau_max = 0.1
    k = 2
    y[k-2], y[k-1], y[k], y[k+1], tau[k-2], tau[k-1], tau[k] = euler()
    R = np.abs(y[k+1] - y[k])
    it = 0
    while t_k < T :
        t_k_next = t_k + tau[k]
        a0 = (tau[k] + tau[k-1])**2 / (tau[k-1]*(2*tau[k]+tau[k-1]))
        a1 = - (tau[k] ** 2) / (tau[k-1]*(2*tau[k]+tau[k-1]))
        b0 = (tau[k] * (tau[k] + tau[k-1])) / (2*tau[k]+tau[k-1])
        y_k = y[k+1]
        y[k+1] = a1 * y[k-1] + a0 * y[k] + b0 * f(y[k+1], t_k_next)

        u_3_t = 6 * (y[k + 1] / (tau[k] * (tau[k] + tau[k-1]) * (tau[k] + tau[k-1] + tau[k-2]))
                     - y[k] / (tau[k] * tau[k-1] * (tau[k-1] + tau[k-2]))
                     + y[k-1] / (tau[k-1] * tau[k-2] * (tau[k] + tau[k-1]))
                     - y[k-2] / (tau[k-2] * (tau[k-1] + tau[k-2]) * (tau[k] + tau[k-1] + tau[k-2])))
        R = (((tau[k] ** 2) * (tau[k] + tau[k-1]) ** 2) / (6 * (2 * tau[k] + tau[k-1]))) * u_3_t
        ...
        tau_k_i = np.zeros([NUMBER_OF_EQUATIONS])
        for i in range(NUMBER_OF_EQUATIONS):
            tau_k_i[i] = eps_dop/(np.abs(y[k+1][i]) + eps_dop / tau_max)

        tau[k] = min(tau_k_i)
        print('K: [{2}] | Y: {0} | T: {1}'.format(y[k+1], t_k_next, it))
        t_k = t_k_next
        it += 1    
